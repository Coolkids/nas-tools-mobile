...
async function loadCategory(cat: DiscoveryCategory) {
  ...
  try {
    const res = await getRecommend({ type: cat.type, subtype: cat.subtype, page: 1, week: cat.week })
    if (res.code === 0) {
...
<template>
  <div class="ranking-view">
    <van-tabs v-model:active="activeTab" @change="switchTab" style="position:sticky;top:0;z-index:10">
