...
export function getTransferHistory(params: { page?: number; keyword?: string; pagenum?: number } = {}): Promise<TransferHistoryResult> {
...
export function deleteHistory(flag: 'del_source' | 'del_dest' | 'del_all', logids: Array<number | string>): Promise<{ code: number; msg?: string }> {
  return doAction('delete_history', { flag, logids })
}

export function reIdentification(flag: 'history' | 'unidentification', ids: Array<number | string>): Promise<{ retcode: number; retmsg: string }> {
...
export function getTmdbCache(params: { page?: number; keyword?: string; pagenum?: number } = {}): Promise<TmdbCacheResult> {
