...
export function rssArticlesDownload(taskid: string | number, articles: Array<{ title: string; enclosure?: string }>): Promise<SimpleResult> {
  return doAction<SimpleResult>('rss_articles_download', { taskid, articles })
}

export function rssArticlesCheck(flag: 'set_finished' | 'set_unfinish', articles: Array<{ title: string; enclosure?: string }>): Promise<SimpleResult> {
  return doAction<SimpleResult>('rss_articles_check', { flag, articles })
}

export function rssArticleTest(taskid: string | number, title: string): Promise<{ code: number; data?: Record<string, unknown> }> {
